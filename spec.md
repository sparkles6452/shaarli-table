...
Conducted against the initial release. Two issues were fixed; one is documented as a known limitation.

### Fixed: `javascript:` URL not blocked (medium)

`htmlspecialchars` escapes HTML entities but does not block the `javascript:` URI scheme. A bookmark stored with `javascript:alert(1)` as its URL would have caused script execution on click. Fixed by validating the URL scheme before rendering: only `http://`, `https://`, `ftp://`, `ftps://`, `magnet:`, and `mailto:` pass through; anything else is replaced with `#`. Shaarli likely rejects such URLs at save time, but the plugin should not rely on that.

### Fixed: date values unescaped in fallback branch (low)

When `$link['created']` is neither a `DateTimeInterface` nor an integer (unexpected string type), the display and sort values were written into HTML attributes without `htmlspecialchars`. Fixed by adding explicit escaping in that branch. In practice Shaarli stores dates as `DateTimeImmutable`, so this branch is rarely reached.

### Known limitation: delete links lack a CSRF token (low/informational)

Shaarli's own linklist template appends `&token={$token}` to delete URLs. The plugin does not have access to this token inside a plugin hook (it is generated by the controller and passed only to templates). As a result:

- If Shaarli enforces the token (expected behaviour), the Delete button will silently fail server-side. The link will appear to navigate but no deletion will occur.
- If Shaarli does not enforce the token for this action, the link is susceptible to CSRF (a third-party page could trigger deletion for a logged-in user by embedding the URL).

Workaround: users should use Shaarli's native card view for delete operations until this is resolved, or a future version of the plugin obtains the token through an alternative mechanism.

### Not an issue: `QUERY_STRING` used for toggle URL

The chain `parse_str → array_merge → http_build_query → htmlspecialchars` fully neutralises any injected content before the value reaches an `href` attribute. The resulting URL is always relative (starts with `?`).

### Not an issue: clipboard copy via textarea fallback

`ta.value = text` sets the DOM property directly, bypassing HTML parsing. No XSS path exists through the clipboard content.
